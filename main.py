...
def create_user():
    ...
    with Session() as session:
        ...
        # Hämta första kolumnen (id)
        new_id = result.fetchone()[0]
        session.commit()

    # Returnera vid lyckad registrering
    return jsonify({"message": f"User '{name}' created successfully", "id": new_id}), 201
...
def get_user(id):
    with Session() as session:
        ...
        result = session.execute(text("SELECT * FROM users WHERE id = :id"), {"id": id}).fetchone()

    if result:
        # Konvertera till en dictionarie
        users = {"id": result.id, "name": result.name, "email": result.email}
        return jsonify(users), 200
    else:
        return jsonify({"message": f"User with id {id} was not found."}), 404


@app.delete('/users/<int:id>')
def remove_user(id):
    with Session() as session:
        result = session.execute(text("DELETE FROM users WHERE id = :id"), {"id": id})
        session.commit()

        # Kontrollera hur många rader som påverkades
        if result.rowcount > 0:
            return jsonify({"message": f"User with id {id} was successfully deleted"}), 200
        else:
